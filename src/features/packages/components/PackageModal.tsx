...
const PackageForm: React.FC<PackageFormProps> = ({
  ...
  const [sellPrice, setSellPrice] = useState<number | ''>(
    () => packageToEdit?.sell_price ?? ''
  );
  const [isActive, setIsActive] = useState(() => packageToEdit?.is_active ?? true);
  ...
  const estimatedSavings =
    standardSeparatePrice > numericSellPrice
      ? standardSeparatePrice - numericSellPrice
      : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (new Set(productIds).size !== productIds.length) {
      setErrorMsg(
        'Terdapat produk duplikat di dalam paket. Harap satukan dalam kuantitas.'
      );
      return;
    }
    ...
    if (numericSellPrice < computedHpp) {
      setErrorMsg(
        `Harga jual paket (${formatRupiah(
          numericSellPrice
        )}) tidak boleh lebih murah dari total HPP modal (${formatRupiah(
          computedHpp
        )})`
      );
      return;
    ...
    <form onSubmit={handleSubmit} className="space-y-4 mt-2">
              ...
              return (
                <div key={idx} className="p-3 flex items-center gap-3 text-xs">
                  ...
                  <div className="w-28 text-right font-mono font-medium text-stone-700">
                    {product
                      ? formatRupiah(product.sell_price * item.quantity)
                      : '-'}
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
        <Input
          ...
          helperText={`Harga normal terpisah: ${formatRupiah(
            standardSeparatePrice
          )}`}
          value={sellPrice}
          onChange={(e) =>
            setSellPrice(e.target.value === '' ? '' : Number(e.target.value))
          }
          required
        />
...
export const PackageModal: React.FC<PackageModalProps> = ({
  ...
  return (
    <Modal
      ...
      title={
        <div className="flex items-center gap-2">
          ...
          <span>
            {packageToEdit ? 'Edit Paket Bundling' : 'Buat Paket Bundling Baru'}
          </span>
        </div>
      }
    ...
    >
      {isOpen && (
        <PackageForm
          key={packageToEdit ? `edit-${packageToEdit.id}` : 'create-new'}
